...
FileNameMinMax = "max"+path[4:16]+".png"
Min=[]
Max=[]
MM=[]
...
for line in raw_data():
    ...
    MM.append(float(t[14:16])+float(t[17:19])/60)
    Min.append(float(min_z))
    Max.append(float(max_z))
...
for t, zs in collate_row(x_size):
    ...
    if old_y != y:
        for x in range(len(average)):
            pix[x,old_y+tape_height] = rgb3(average[x]/tally)
        tally = 0
        average = [0.0] * len(freqs)
    old_y = y
    for x in range(len(zs)):
...
if args.time_tick:
    margin = 60
shadow_text(margin, img.size[1] - 45, 'Duration: %i:%02i hh:mm' % (hours, minutes), font)
...
plt.plot(Min,MM,'bo',label='min')
plt.plot(Max,MM,'ro',label='max')
plt.legend( loc='upper right', numpoints=1 )
x1,x2,y1,y2 = plt.axis()
plt.axis([-60, 0,max(MM),0])
plt.subplots_adjust(left=0.1, right=0.93, top=0.93, bottom=0.02)
plt.grid()
plt.savefig(FileNameMinMax,dpi=100)
